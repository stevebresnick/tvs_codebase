...
class H5peditorFile {
  ...
  function __construct($interface, $files_directory) {
    ...
    $dirs = array ('', '/files', '/images', '/videos', '/audios','/pdfs','/ppts');
    foreach ($dirs as $dir) {
  ...
  public function validate() {
    ...
    switch ($this->field->type) {
      ...
      case 'video':
        ...
        if (!$this->check($allowed)) {
          $this->result->error = $this->interface->t('Invalid video file format (use MP4).');
          return FALSE;
        }
 ...
 public function copy() {
    ...
    if (($this->field->type == "pdf") || ($this->field->type == "ppt")) {
        $rootDir = $this->files_directory . '/' . $this->field->type . 's/';
        if (!is_dir($rootDir)) {
            if (!mkdir($rootDir)) {
                return;
            }
          }
        chmod ($this->files_directory . '/',0777);
    }
