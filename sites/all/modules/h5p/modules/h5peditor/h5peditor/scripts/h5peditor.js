function nextWorkflowStep() {
    var workflowMain = H5P.jQuery(".workflowstep");
}
...
//For workflows, track what step we are on...
...
ns.processSemanticsChunk = function (semanticsChunk, params, $wrapper, parent) {
  ...
  for (var i = 0; i < semanticsChunk.length; i++) {
    ...
    var currentStep = Number(localStorage.getItem("currentWorkflowStep"));
    if ((parent.field) && (parent.field.type) && (parent.field.type === "workflow") && (currentStep !== (i+1))) {
        //If this is not the current workflow step make sure the section is hidden
        displayItem = false;
    }
    
    fieldInstance.appendTo($wrapper,displayItem);
