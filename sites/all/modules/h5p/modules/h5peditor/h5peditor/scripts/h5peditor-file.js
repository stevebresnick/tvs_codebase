...
ns.File.prototype.appendTo = function ($wrapper) {
  ...
  if (this.field.label !== 0) {
    ...
    if ((this.field.type === 'pdf') || (localStorage.getItem("tvs_primary_app").toLowerCase() === "direct")) {
        html = ns.createItem(this.field.type, label + '<span class="fileThumb"></span><div role="button" tabindex="1" class="h5p-add-file" title="' + H5PEditor.t('core', 'addFile') + '"></div><div class="h5p-add-dialog"><div class="h5p-dialog-box"><button class="h5p-file-upload">Upload new file</button></div><div class="h5p-buttons"><button class="h5p-cancel">Cancel</button></div></div>' +  copyRightHtml, this.field.description);
    }
...
ns.File.prototype.uploadFile = function () {
      ...
      if (err) {
        ...
        if (that.field.type === 'image') {   
            ...
            H5P.jQuery.ajax({ 
                ...
                success: function(data){
                    debugger;
                    var filePath = H5P.getPath(result.path, H5PEditor.contentId);
                    if (that.field.type === 'ppt') {
                        filePath = "/sites/all/libraries/" + result.path;
                        filePath = filePath.replace(".pptx","");
                        filePath = filePath.replace(".ppt","");
                        filePath += "/index.html";
                    }
                    that.params = {
                        path: filePath,
  ...
  if (this.field.mimes !== undefined) {
    ...
    ns.File.$file.attr('accept', 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
  }
