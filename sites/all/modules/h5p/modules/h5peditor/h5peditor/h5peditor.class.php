...
class H5peditor {
  ...
  private function processField(&$field, &$params, &$files) {  
    //error_log("Field type is " . $field->type, 0);
    //error_log("Field name is " . $field->name, 0);

    if (($field->type === "workflow") && ($field->name === "interactiveVideo")) {
        $field = $field->steps->fields;
        $params = $params->video->files;
        $field->type = "video";
    }
    else if (($field->type === "workflow") && ($field->name === "menu")) {
        $field = $field->steps->fields;
        $field->type = "video";
        error_log("Number of hotspots: " . sizeof($params->hotspots),0);
        for ($i = 0; $i < sizeof($params->hotspots); $i++) {
            $this->processField($field, $params->hotspots[$i]->action->params->sources, $files);
        }
        return;
    }
    else if (($field->type === "workflow") && ($field->name === "MLE")) {
        $field = $field->steps->fields;
        $params = $params->content->params->sources;
        $field->type = "video";
    }
    ...
    switch ($field->type) {
      ...
      case 'workflow':
        if ($field->name === "MLE") {
          for ($j = 0, $t = count($params->steps); $j < $t; $j++) {
            $this->processField($field->steps, $params[$j], $files);
          }
        }
        break;
    }
  ...
  private function processFile(&$params, &$files) {
    ...
    error_log( "Process File: " . $params->path, 0);
      
    if (preg_match($this->relativePathRegExp, $params->path, $matches)) {
     error_log( "Preg_match matched", 0);
      // Create copy of file
      $source = $this->content_directory . $params->path;
      $destination = $this->content_directory . $matches[3];
      error_log( "Source is : " . $source, 0);
      error_log( "Destination is : " . $destination, 0);
      if (file_exists($source) && !file_exists($destination)) {
        $logMsg = "Src1: " . $source . ";Dest1: " . $destination;
        error_log($logMsg,0);
        copy($source, $destination);
      }
      $this->storage->keepFile($destination, $destination);
      $params->path = $matches[3];
    }
    else {
        $exists = false;
        error_log( "Preg_match did not match", 0);
        if (strpos($params->path, 'https://') !== false) {
          $oldPath = $params->path;
          $newPath = $params->path;  
          
          $file_headers = @get_headers($params->path);
      ...
      error_log( "NewPath is : " . $newPath, 0);
      error_log( "OldPath is : " . $oldPath, 0);
      if (file_exists($newPath) || ($exists == true)) {
        // Uploaded to content folder, make sure the cleanup script doesn't get it.
        $logMsg = "Src2: " . $oldPath . ";Dest2: " . $newPath;
        error_log($logMsg,0);
        $this->storage->keepFile($newPath, $newPath);
      }
      elseif (file_exists($oldPath)) {
        // Copy file from editor tmp folder to content folder
        $logMsg = "Src3: " . $oldPath . ";Dest3: " . $newPath;
        $this->storage->keepFile($oldPath, $oldPath);
        error_log($logMsg,0);
        $chmodCmd = "sudo chmod 777 " . $newPath;
        system($chmodCmd);
    ...
    //Check if this is the video that generates the thumbnail image.
    $thumbnailSourceFilePath = str_replace("videos", "images", $oldPath);
    $thumbnailSourceFilePath = str_replace("files-", "thumbnail-", $thumbnailSourceFilePath);
    $thumbnailSourceFilePath = str_replace(".mp4", ".jpg", $thumbnailSourceFilePath);
    $thumbnailDestFilePath = substr($newPath,0,strpos($newPath,'videos/')) . "images/thumbnail.jpg";
    
    $logMsg = "Src5: " . $thumbnailSourceFilePath . ";Dest5: " . $thumbnailDestFilePath;
    error_log($logMsg,0);
    if (file_exists($thumbnailSourceFilePath)) {
        $logMsg = "Src6: " . $thumbnailSourceFilePath . ";Dest6: " . $thumbnailDestFilePath;
        error_log($logMsg,0);
        copy($thumbnailSourceFilePath, $thumbnailDestFilePath);
    }
