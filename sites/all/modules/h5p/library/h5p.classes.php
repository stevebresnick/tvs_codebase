...
interface H5PFrameworkInterface {
  ...
  public function createDownloadDirectory();
...
class H5PContentValidator {
  ...
  private static $allowed_stylable_tags = array('strong', 'b', 'em', 'i', 'u', 'del', 'h1', 'h2', 'h3', 'a', 'ul', 'ol', 'li', 'table', 'hr', 'tr', 'td','th','div','blockquote','span','font','p','br');
  ...
  public function validateText(&$text, $semantics) {
    ...
    if (!($text === '' && isset($semantics->optional) && $semantics->optional) && isset($semantics->regexp)) {
      ...
      if (preg_match($pattern, $text) === 0) {
        ...
        $this->h5pF->setErrorMessage($this->h5pF->t('Provided string is not valid. (value: "%value", regexp: "%regexp")', array('%value' => $text, '%regexp' => $pattern)));
        $text = '';
      }
    }
  }
   ...
   * Will recurse into validating each item in the list according to the type.
   */
  public function validateWorkflow(&$workflow, $semantics) {
  ...
  private function _validateFilelike(&$file, $semantics, $typevalidkeys = array()) {
    ...
    // code.
    $validkeys = array_merge(array('path', 'mime', 'copyright','damId'), $typevalidkeys);
  ...
  public function validateLibrary(&$value, $semantics) {
    ...
    $validkeys = array('library', 'params', 'subContentId','menu','settings','hotspots');
    if (isset($semantics->extraAttributes)) {
   ...
   */
  private function filter_xss($string, $allowed_tags = array('strong', 'b', 'em', 'i', 'u', 'del', 'h1', 'h2', 'h3', 'a', 'ul', 'ol', 'li', 'table', 'hr', 'tr', 'td','th','div','blockquote','span','font','br')) {
    if (strlen($string) == 0) {
      return $string;
    }
  ...
  private function _filter_xss_split($m, $store = FALSE) {
    ...
    return "<$elem$attrlist$xhtml_slash>";
  }
